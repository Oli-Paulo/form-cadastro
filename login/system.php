...
<body class="bg-gray-100 h-screen flex overflow-hidden">
    ...
    <div id="userModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full hidden">
        <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
            <div class="mt-3 text-center">
                ...
                <form id="userForm" class="mt-2 text-left" action="interface.php" method="POST">
                <input type="hidden" name="path" value="system.php">
    ...
    <?php
        include 'Connect.php'; // Sera usada pela classe Usuario
        include 'Usuario.php';  //adicionar a classe Usuario
    ...
    <script>
        var exampleData = [
            ...
            foreach ($usuarios as $usuario) {
                echo "{ id: {$usuario['id']}, nome: '{$usuario['nome']}', email: '{$usuario['email']}', telefone: '{$usuario['telefone']}', cpf: '{$usuario['cpf']}' },";
            }
            ?>
        ];
    ...
    $('#id').val('');  // Limpa o ID para novos cadastros
    $('#formMode').val('add');  // Define o modo para add
    $('#userForm')[0].reset();
    ...
    if (user) {
        ...
        $('#id').val(user.id);
        $('#formMode').val('edit');  // Define o modo para edit
        $('#nome').val(user.nome);
        ...
        if(confirm('Tem certeza que deseja deletar o usuário ' + id + '?')) {
            //Redirecionar para meu script php com o ID do ususario para deletar
            window.location.href = "interface.php?action=delete&id="+id;            
        }
    }
